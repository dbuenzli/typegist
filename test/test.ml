...
let test_pair () =
  ...
  log "* Testing pairs";
  log "   type: @[%a@]" Type.Gist.pp_type pair;
  log "  value: @[%a@]" (Fun.Generic.pp pair) (3, "hey");
  log ""
...
let test_func () =
  ...
  let pair x y = x, y in
  log "* Testing function representation";
  log "  value String.length: @[%a@]" (Fun.Generic.pp func) String.length;
  log "  value pair constructor: @[%a@]" (Fun.Generic.pp ret_pair) pair;
  log ""
...
let test_btree () =
  ...
  log "* Testing Btree";
  log "   type: @[%a@]" Type.Gist.pp_type int_tree;
  log "  value: @[%a@]" (Btree.pp Type.Gist.int) t;
  log ""
...
let test_abstract_person () =
  ...
  log "* Testing Person";
  log "   type: @[%a@]" Type.Gist.pp_type Person.type_gist;
  log "  value: @[%a@]" Person.pp p;
  log ""
...
let test_maplike () =
  ...
  log "* Testing maplike";
  log "   type: @[%a@]" Type.Gist.pp_type g;
  log "  value: @[%a@]" (Fun.Generic.pp g) imap;
  log ""
...
let test_rec () =
  ...
  log "* Testing mutually recursive";
  log "  type one: @[%a@]" Type.Gist.pp_type M.type_gist_one;
  log "  type two: @[%a@]" Type.Gist.pp_type M.type_gist_two;
  log "     value: @[%a@]" (Fun.Generic.pp M.type_gist_one) v;
  log ""
...
let test_custom_fmt () =
  ...
  log "* Testing custom formatter";
  log " normal: @[%a@]" (Fun.Generic.pp uchars) us;
  log " custom: @[%a@]" (Fun.Generic.pp uchars_debug) us;
  log ""
...
let test_gadt () =
  ...
  log "* Testing an existential GADT";
  log "  type: @[%a@]" Type.Gist.pp_type M.type_gist;
  log "  type: @[%a@]" Type.Gist.pp_type M.type_gist;
  log " value: @[%a@]" (Fun.Generic.pp M.type_gist) v;
  log ""

let test_key () =
  log "* Testing meta keys";
