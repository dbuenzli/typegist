...
(** Type introspection.
    ...
    {b Warning.} Typegist is unstable, see {{!design_notes}design notes} and
    {{!todo}todos}.

    The [typegist] library:

    {ul
    {- Provides the [Stdlib.Type] module available in OCaml 5.1 with an
       additional {!Type.Gist} submodule for reflecting OCaml types as values.}
    {- Extends the [Stdlib.Fun] module with a {!Fun.Generic} module which
       has a few sample generic functions.}}

    See {{!page-quick.examples}description examples} and
    a {{!page-quick.function_template}generic function template}.
...
(** {1:design_notes Design notes}
    ...
    Similar to {{:https://arxiv.org/abs/1812.11665}Balestrieri et
    al.'s} low level view (§2.3.4). However, the representation is not
    extensible, a few case are dropped, some representations are unified,
    some specializations are added.

    {ul
    {- Products, records and variant (cases) are distinguished at the toplevel
       but they all share a single representation: a {e product of
       fields representation}. An economical uniformity that benefits both
       processors and the programmer's mind.}
    {- The {{!Typegist.Type.Gist.product_ops}product of fields
       ...
       providing conversion to/from a multi-dimensional tuple,
       the user provides field projectors and a constructor that
       creates values from the fields. This representation is rather natural
       and easy to explain. It's a good fit for abstract records since APIs
       usually already provide a constructor and field accessors.  It
       is however more painful for variant cases which are usually
       deconstructed via pattern matching, having a quick way of
       devising these projectors would be useful.}
    {- The {{!Typegist.Type.Gist.abstract_ops}representation of
       abstract types} can expose more than one public representation
       (or none). Public representations have a version label which
       allows to interact with systems still using the previous
       representation.}
    ...
    {- Balance between specialisation and generalisation. We have
       ...
       with {!Type.Gist.Sum.to_variant}. Similar schemes are provided
       by {!Type.Gist.maplike}, {!Type.Gist.arraylike}.
       Scalar values are all grouped under {!Type.Gist.scalar} for which
       a few generic operations are provided in {!Type.Gist.module-Scalar}.}
    ...
    {- Representation of variants. The internal data structure is a bit too
       naive. Both {!Fun.Generic.compare} and {!Fun.Generic.random} are
       quite inefficient.}}
    ...
    end I shifted them back into the subvariants themselves despite the
    types looking heavier ({!Type.Gist.scalar}, {!Type.Gist.sum},
    etc.). It makes the types meta-aware without needing to carry
    something on the side. The drawback is that generic meta lookup on
    a gist is likely slower.
...
(*---------------------------------------------------------------------------
   Copyright (c) 2023 The typegist programmers
